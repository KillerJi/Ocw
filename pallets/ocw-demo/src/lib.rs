...
decl_storage! {
	trait Store for Module<T: Trait> as Example {
		...
		Prices get(fn prices): VecDeque<PriceData>;
	}
}
...
decl_event!(
		...
		AccountId = <T as system::Trait>::AccountId,
	{
		...
		NewPrice(Option<AccountId>, PriceData),
	}
);
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		#[weight = 0]
		pub fn submit_dot_price_unsigned(origin, price: PriceData) {
			let _ = ensure_none(origin)?;
			debug::info!("submit_dot_price_unsigned: ({:?})", price);
...
impl<T: Trait> Module<T> {
	...
	fn append_or_replace_price(price: PriceData) {
	...
	fn fetch_dot_info_action() -> Result<(), Error<T>> {
		...
		if let Ok(_guard) = lock.try_lock() {
			match Self::fetch_dot_info_parse() {
				Ok(price) => { 
					if let Some(Some(old_price)) = s_info.get::<PriceData>() {
						// dot-info has already been fetched. Return early.
						if price.iter().zip(old_price.iter()).all(|(a,b)| a == b) {
							debug::info!("cached price: {:?}", price);
							return Ok(());
						}
					}
					s_info.set(&price);
					let call = Call::submit_dot_price_unsigned(price);
	...
	fn fetch_dot_info_parse() -> Result<PriceData, Error<T>> {
		...
		let price_usd = json["data"]["priceUsd"].as_str().ok_or(<Error<T>>::HttpFetchingError)?;
		debug::info!("{}", price_usd);
		let mut price_usd: PriceData = price_usd.as_bytes().to_vec();
		if price_usd.len() > 32 {
...
impl<T: Trait> frame_support::unsigned::ValidateUnsigned for Module<T> {
	...
	fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
		...
		match call {
			Call::submit_dot_price_unsigned(_price) => valid_tx(b"submit_dot_price_unsigned".to_vec()),
